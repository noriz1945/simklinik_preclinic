...
class Frm_kpk_nicu extends MX_Controller {
	var $session_name='sp';
	...
	function __construct() {
		...
		$this->load->model('Mdl_kpk_nicu','mdl');
	}
  ...
  public function frmasm($id_reg){
	...
	$this->load->view('frm_kpk_nicu', $data);
  }
...
//NEWS
  public function mst(){
	...
	,COUNT(id)+1 AS total FROM frm_mst_kpk_nicu GROUP BY grup_id ORDER BY grup_id ASC";
	...
	(SELECT nilai FROM frm_trn_kpk_nicu WHERE a.id=id_mst_news AND id_reg='$id_reg') AS nilai_from_trn_1
	FROM frm_mst_kpk_nicu a 
...
//End NEWS
 ...
 public function mst_rep(){
    ...
    //lvl 2
    $sql_3 = "SELECT
	(SELECT nilai FROM frm_trn_kpk_nicu WHERE id_mst_news='$id_mst_news' AND id_reg='$id_reg' GROUP BY id_mst_news) AS nilai_1
	FROM frm_trn_kpk_nicu 
