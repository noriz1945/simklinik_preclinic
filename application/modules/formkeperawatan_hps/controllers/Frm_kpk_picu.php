...
class Frm_kpk_picu extends MX_Controller {
	var $session_name='sp';
	...
	function __construct() {
		...
		$this->load->model('Mdl_kpk_picu','mdl');
	}
  ...
  public function frmasm($id_reg){
	...
	$this->load->view('frm_kpk_picu', $data);
  }
...
//NEWS
  public function mst(){
	...
	,COUNT(id)+1 AS total FROM frm_mst_kpk_picu GROUP BY grup_id ORDER BY grup_id ASC";
	...
	(SELECT nilai FROM frm_trn_kpk_picu WHERE a.id=id_mst_news AND id_reg='$id_reg') AS nilai_from_trn_1
	FROM frm_mst_kpk_picu a 
...
//End NEWS
 ...
 public function mst_rep(){
    ...
    //lvl 2
    $sql_3 = "SELECT
	(SELECT nilai FROM frm_trn_kpk_picu WHERE id_mst_news='$id_mst_news' AND id_reg='$id_reg' GROUP BY id_mst_news) AS nilai_1
	FROM frm_trn_kpk_picu 
