...
<div class="content">
    ...
    <div class="d-md-flex align-items-center justify-content-between mb-4">
        <div>
            <h4 class="fw-bold mb-0 text-dark">Master Spesialisasi Dokter</h4>
            <p class="text-muted mb-0">Daftar spesialisasi medis yang tersedia untuk tenaga ahli di klinik.</p>
        </div>
        <div class="mt-3 mt-md-0">
            <button type="button" class="btn btn-primary d-inline-flex align-items-center shadow-sm"
                data-bs-toggle="modal" data-bs-target="#modalAdd">
                <i class="ti ti-plus me-1"></i> Tambah Spesialisasi
            </button>
        </div>
    </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" id="custom-search" class="form-control bg-light border-0 shadow-none" 
                            placeholder="Ketik nama spesialisasi...">
                    </div>
                </div>
                <div class="col-md-3">
    ...
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-header bg-white border-bottom-0 py-3">
            <h5 class="card-title mb-0 d-flex align-items-center">
                <i class="ti ti-certificate text-primary me-2 fs-20"></i> Daftar Spesialisasi
            </h5>
        </div>
                ...
                <table class="table table-hover align-middle mb-0 datatable">
                    <thead class="bg-light bg-opacity-50">
                        <tr>
                            ...
                            <th class="border-0 py-3 text-muted fs-12 text-uppercase fw-bold">Nama Spesialisasi</th>
                            <th class="border-0 py-3 text-muted fs-12 text-uppercase fw-bold" width="120">Status</th>
                            <th class="border-0 pe-3 py-3 text-center text-muted fs-12 text-uppercase fw-bold" width="100">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php $no = 1;
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0">
                                            ...
                                            <?php if ($dt->aktif == 1): ?>
                                                <li><a class="dropdown-item py-2 text-danger" href="javascript:void(0)"
                                                        onclick="toggleStatus('<?= $dt->id_spes ?>', '<?= htmlspecialchars($dt->name) ?>', 'nonaktif')">
                                                        <i class="ti ti-power me-2 fs-16"></i> Nonaktifkan</a>
                                                </li>
                                            <?php else: ?>
                                                <li><a class="dropdown-item py-2 text-success" href="javascript:void(0)"
                                                        onclick="toggleStatus('<?= $dt->id_spes ?>', '<?= htmlspecialchars($dt->name) ?>', 'aktif')">
        ...
        <div class="card-footer bg-transparent border-0 py-3">
            <p class="text-muted fs-11 mb-0"><i class="ti ti-info-circle me-1 text-primary"></i> Data spesialisasi ini akan digunakan untuk pengelompokan layanan dokter.</p>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title d-flex align-items-center"><i class="ti ti-plus me-2"></i> Tambah Spesialisasi
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form action="<?= base_url('mst_dokter/save_spesialis_dokter/') ?>" method="POST">
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-bold fs-13">Nama Spesialisasi</label>
                        <input type="text" class="form-control bg-light border-0 shadow-none" name="nama_spesialis_dokter"
                            placeholder="Cth: Penyakit Dalam, Anak, dll" required>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-info text-white">
                <h5 class="modal-title d-flex align-items-center"><i class="ti ti-edit me-2"></i> Edit Spesialisasi</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form action="<?= base_url('mst_dokter/editthis_spesialis_dokter/') ?>" method="POST">
                <div class="modal-body p-4">
                    <input type="hidden" name="id" id="edt_id">
                    <div class="mb-3">
                        <label class="form-label fw-bold fs-13">Nama Spesialisasi</label>
                        <input type="text" class="form-control bg-light border-0 shadow-none" name="edt_nama_spesialis_dokter"
                            id="edt_name" required>
                    </div>
                </div>
                <div class="modal-footer bg-light border-0">
                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">Batal</button>
...
<script>
    ...
    function toggleStatus(id, name, action) {
        var title = action == 'nonaktif' ? 'Nonaktifkan Spesialisasi?' : 'Aktifkan Spesialisasi?';
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var url = action == 'nonaktif' ? "<?= base_url('mst_dokter/deleteitempo_spesialis_dokter/') ?>" : "<?= base_url('mst_dokter/aktifasiitempo_spesialis_dokter/') ?>";
