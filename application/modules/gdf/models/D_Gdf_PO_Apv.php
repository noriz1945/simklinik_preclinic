<?php
class D_Gdf_PO_Apv extends ci_model
{
  ////////////////////////////
  ...
  function list_obat_per_norpo_depo($no_rpo_p, $id_wrh_set){
    $query=$this->db->query("SELECT a.*,b.name AS nama_obat,a.qty_last AS qty_soh,a.qty_depo AS qty_depo,d.closedate,b.konversi,e.name AS satuan
    FROM gdf_rpo_det a
    LEFT JOIN mst_farmalkes b ON a.id_obat=b.id_fa
    LEFT JOIN mst_soh c ON (a.id_obat=c.id_soh AND c.id_wrh='$id_wrh_set')
    LEFT JOIN gdf_rpo d ON a.id_rpo=d.id_rpo
  ...
  function proses_soh($nosto){ 
    $query=$this->db->query("SELECT a.from AS ke_gudang,a.request_date,b.*,c.name AS nama_obat,d.name AS nama_gudang
    FROM gdf_rpo a 
    LEFT JOIN gdf_rpo_det b ON (a.id_rpo=b.id_rpo)
    LEFT JOIN mst_farmalkes c ON b.id_obat=c.id_fa
    LEFT JOIN mst_warehouse d ON a.from=d.id_wrh
    WHERE a.id_rpo='$nosto' AND a.status='0'");
    return $query->result(); 
  }
  ...
  function get_data_obat_from_rpo_det($id){
    $query=$this->db->query("SELECT * FROM gdf_rpo_det WHERE id='$id'");
    return $query->row();
  }
  //END KARTU STOK
} 
