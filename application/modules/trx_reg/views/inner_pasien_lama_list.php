																			...
																			<tbody>
																				...
																				<tr>
																					...
																					<td nowrap> 
																						...
																						?> 
																						<a onclick="javascript : reg_pasien_lama_aps('<?php echo $mst_pasien->id_pasien; ?>');" href="#"><button type="button" class="btn btn-success waves-effect">Reg. APS </button></a>
																					</td>
...
{
	$.ajax({
		url: "<?php echo site_url('trx_reg/has_booking_today_ajax'); ?>",
		method: "POST",
		dataType: "json",
		data: {id_pasien: id_pasien},
		success: function(res){
			if(res && res.status && res.has){
				$("#tab_pas_lama").load("<?php echo site_url('trx_reg/inner_pasien_lama_reg/'); ?>"+id_pasien);
			}else{
				alert('Pasien belum mempunyai booking hari ini. Silakan lakukan booking terlebih dahulu.');
				window.location.href = "<?php echo site_url('booking'); ?>";
			}
		},
		error: function(){
			alert('Gagal memeriksa status booking pasien. Coba lagi.');
		}
	});
}

function reg_pasien_lama_aps(id_pasien)
{
	$.ajax({
		url: '<?php echo site_url('trx_reg/inner_pasien_aps_reg/'); ?>' + id_pasien,
		dataType: 'html',
		success: function(html) {
			$('#tab_pas_aps').html(html);
			
			$("#id_pas_aps").addClass("active");
			$("#id_pas_lama").removeClass("active");

			$("#id_pas_aps").addClass("show");
			$("#id_pas_lama").removeClass("show");
            
            $("#tab_pas_aps").addClass("active");
			$("#tab_pas_lama").removeClass("active");

			$("#tab_pas_aps").addClass("show");
			$("#tab_pas_lama").removeClass("show");
		}
	});	
}
