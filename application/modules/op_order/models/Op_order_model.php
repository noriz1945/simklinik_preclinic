...
class Op_order_model extends CI_Model
{
  ...
  function get_data_registrasi_by_id_reg($id_reg)
    ...
    {
        $sql = "SELECT  * FROM `transaksi_pendaftaran`  WHERE  `no_rekam_medis` = '$id_reg'";
        $query  = $this->dbhis->query($sql);
        $rs     = $query->row_array();
        return $rs;
    }

    function views_formorderop($id_pend){
      $query=$this->db->query("SELECT  * FROM `transaksi_pendaftaran`  WHERE  `id_pendaftaran` = '$id_pend'");
      return $query->result();
    }

    function views_formorderop_konsul($id_pend){
      $query=$this->db->query("SELECT  * FROM `transaksi_konsultasi`  WHERE  `id_pendaftaran` = '$id_pend'");
      return $query->result();
    }
    ...
    public function get_mst_unit($id_units)
  {
    $this->db->where('id_unit', $id_units);
    return $this->db->get('mst_unit')->row();
  }

  public function get_mst_tindakan($id_act_post)
  {
    $this->dbhis->where('id_act', $id_act_post);
    return $this->dbhis->get('mst_tindakan')->row();
  }


    function delete_konsultasi_id_kosong($id_pend){
      $this->db->where('id_pendaftaran', $id_pend);
      $this->db->delete('transaksi_konsultasi');
      }

      function delete_op_id($id_pend){
        $this->dbhis->where('id_pendaftaran', $id_pend);
        $this->dbhis->delete('transaksi_pendaftaran');
      }

    function list_order_operasi($id_pasien)
	{

	 $query=$this->db->query("SELECT * FROM `transaksi_pendaftaran`  WHERE `id_pasien`='$id_pasien'");
	 return $query;
	}

  //tpp
  function mst_trx_edit_tpp($id_reg)
  {
    $query=$this->db->query("SELECT * FROM `transaksi_pendaftaran` a 
    LEFT JOIN transaksi_konsultasi b ON a.id_pendaftaran=b.id_pendaftaran  
    WHERE a.no_rekam_medis='$id_reg'
    ORDER BY a.cdd DESC");
    return $query->result(); 
  }
  //tpp
