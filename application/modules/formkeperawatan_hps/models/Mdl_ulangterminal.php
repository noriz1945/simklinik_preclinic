...
class Mdl_ulangterminal extends CI_Model
{
  ...
  public function add_data_asm_ranap_ott($data_ott){
    $this->dbsupp->insert('frm_asm_terminal', $data_ott);
  } 


  public function edit_data_asm_ranap_ott($where, $data){
    $this->dbsupp->update('frm_asm_terminal', $data, $where);
    return $this->dbsupp->affected_rows();
  }

  
  public function getlogterminal($id_reg){
    $sql ="SELECT * FROM frm_asm_terminal WHERE id_reg='$id_reg' AND status='0' ORDER by tgl_asulang DESC";
    $query  = $this->dbsupp->query($sql);
    $result = $query->result_array();
    return $result;
  }

  public function getdetaillogterminal($id){
    $sql ="SELECT * FROM frm_asm_terminal WHERE id='$id'";
    $query  = $this->dbsupp->query($sql);
    $result = $query->row();
    return $result;
  }
