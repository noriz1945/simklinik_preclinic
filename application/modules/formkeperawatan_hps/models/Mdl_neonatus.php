...
class Mdl_neonatus extends CI_Model
{
  ...
  {
    $sql ="SELECT COUNT(*) AS fnddata FROM frm_asm_neonatus WHERE id_reg='$id_reg'";
  ...
  public function add_data_asm_ranap_ott($data_ott){
    $this->dbsupp->insert('frm_asm_neonatus', $data_ott);
  } 
  ...
  public function edit_data_asm_ranap_ott($where, $data){
    $this->dbsupp->update('frm_asm_neonatus', $data, $where);
    return $this->dbsupp->affected_rows();
  }
  ...
  {
    $query=$this->dbsupp->query("SELECT * FROM soap_asm_ri a LEFT JOIN frm_asm_neonatus b ON a.id_asmri=b.id_asmri_from WHERE a.id_asmri='$id_asmri'");
    return $query->row();
