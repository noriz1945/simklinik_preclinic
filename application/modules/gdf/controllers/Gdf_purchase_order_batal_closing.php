<?php
class Gdf_purchase_order_batal_closing extends MX_controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('D_Gdf_purchase_order_batal_closing');
    ...
    function index(){
        $this->load->view('list_purchase_order_batal_closing');
    }

    function ppnmargin(){
        $parameter_ppn_margin   = $this->D_Gdf_purchase_order_batal_closing->ppn_set();
    ...
    function listdatarpo(){
        ...
        $data                    = $this->D_Gdf_purchase_order_batal_closing->dlistrpo($tglmulai_set,$tglakhir_set);
    ...
    function edit(){
        ...
        $datasett   = $this->D_Gdf_purchase_order_batal_closing->dlistrpo_edt($no_rpo);
        $row_1      = $datasett->from;
    ...
    function edit_tab(){
        ...
        $data           = $this->D_Gdf_purchase_order_batal_closing->list_obat_per_norpo_edt($nosto);
		$dataset = json_encode($data);
		echo $dataset;
    }    
    ...
    function editthis_apv(){
        ...
        $tgl_batal_closing     = date('Y-m-d');
        $tglwkt_batal_closing  = date('Y-m-d H:i:s');

                ///kartu stok
        //gen no sto
        $checkid                       = $this->D_Gdf_purchase_order_batal_closing->cdk_kartu_stok();
        ...
        $proses_soh_data = $this->D_Gdf_purchase_order_batal_closing->proses_soh($id_proses_penerimaan);
        foreach($proses_soh_data as $dataset){
            ...
            //KARTU STOK 
            //STO
            $kartu_stok_sto = $this->D_Gdf_purchase_order_batal_closing->get_data_obat($id_obat, $id_wrh);
            ...
            $this->D_Gdf_purchase_order_batal_closing->ins2($datains2,'gdf_kartu_stok');
        ...
        //end gen no sto
        ///////////////////
        $total_id_penerimaan        = $this->D_Gdf_purchase_order_batal_closing->get_data_count($nosto);
        $fnndata_total_penerimaan   = $total_id_penerimaan->fnddata;

        if($fnndata_total_penerimaan==0){
            $sql_0 = "UPDATE gdf_purchase_order SET status='0',tanggal_batal_closing='$tgl_batal_closing',updated='$datetime',updated_by='$username' WHERE id_purchase_order='$nosto'";
            $this->db->query($sql_0);

            $sql = "UPDATE gdf_purchase_order_det SET status='1',tgl_batal_closing='$tglwkt_batal_closing',updated='$datetime',updated_by='$username' WHERE id_proses_penerimaan='$id_proses_penerimaan'";
            $this->db->query($sql);
        }else{
            $sql = "UPDATE gdf_purchase_order_det SET status='1',tgl_batal_closing='$tglwkt_batal_closing',updated='$datetime',updated_by='$username' WHERE id_proses_penerimaan='$id_proses_penerimaan'";
            $this->db->query($sql);
    ...
    function detailpenerimaanbarang_closing(){
        ...
        $datasett   	= $this->D_Gdf_purchase_order_batal_closing->detail_closing($id);
    ...
    function listdetailpenerimaanbarang_closing(){
        ...
        $datasett   	= $this->D_Gdf_purchase_order_batal_closing->list_detail_closing($id_por);
        $dataset 		= json_encode($datasett);
        echo $dataset;
    }
