...
class Frm_kpk_icu extends MX_Controller {
	var $session_name='sp';
	...
	function __construct() {
		...
		$this->load->model('Mdl_kpk_icu','mdl');
	}
  ...
  public function frmasm($id_reg){
	...
	$this->load->view('frm_kpk_icu', $data);
  }
...
//NEWS
  public function mst(){
	...
	,COUNT(id)+1 AS total FROM frm_mst_kpk_icu GROUP BY grup_id ORDER BY grup_id ASC";
	...
	(SELECT nilai FROM frm_trn_kpk_icu WHERE a.id=id_mst_news AND id_reg='$id_reg') AS nilai_from_trn_1
	FROM frm_mst_kpk_icu a 
...
public function mst_rep(){
    ...
    //lvl 2
    $sql_3 = "SELECT
	(SELECT nilai FROM frm_trn_kpk_icu WHERE id_mst_news='$id_mst_news' AND id_reg='$id_reg' GROUP BY id_mst_news) AS nilai_1
	FROM frm_trn_kpk_icu 
