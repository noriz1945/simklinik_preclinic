...
class Frm_kpm_picu extends MX_Controller {
	var $session_name='sp';
	...
	function __construct() {
		...
		$this->load->model('Mdl_kpm_picu','mdl');
	}
  ...
  public function frmasm($id_reg){
	...
	$this->load->view('frm_kpm_picu', $data);
  }
...
public function mst(){
	...
	(CASE 
	WHEN grup_id='1' THEN 'PRIORITAS 1 <br> Kelompok pasien sakit kritis, tidak stabil yang memerlukan terapi intensif dan tertirasi, seperti dukunga / bantuan ventilasi, alat penunjang sistem / organ lain, infus obat-obat vasoaktif / inotropic, obat anti aritmia serta pengobatan lain secara kontinyu dan tertitrasi'
	WHEN grup_id='2' THEN 'PRIORITAS 2 <br> Pasien yang memerlukan pelayanan pemantauan canggih di PICU'
	WHEN grup_id='3' THEN 'PRIORITAS 3 <br> Pasien sakit yang tidak stabil status kesehatan sebelumnya yang disebabkan penyakit yang mendasari atau penyakit akutnya secara sendiri atau kombinasi'
	ELSE '-'
	END
	) AS nama_grup
	,COUNT(id)+1 AS total FROM frm_mst_kpm_picu GROUP BY grup_id ORDER BY grup_id ASC";
	...
	(SELECT nilai FROM frm_trn_kpm_picu WHERE a.id=id_mst_news AND id_reg='$id_reg') AS nilai_from_trn_1
	FROM frm_mst_kpm_picu a 
...
public function mst_rep(){
    ...
    //lvl 2
    $sql_3 = "SELECT
	(SELECT nilai FROM frm_trn_kpm_picu WHERE id_mst_news='$id_mst_news' AND id_reg='$id_reg' GROUP BY id_mst_news) AS nilai_1
	FROM frm_trn_kpm_picu 
