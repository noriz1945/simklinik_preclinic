...
class Erm_ranap_model extends CI_Model
{
  ...
  {
    $sql ="SELECT 	TL.id_reg, TR.id_pasien, CONCAT(MP.name,IFNULL(CONCAT(', ',MPT.abbr),'')) AS nama_pasien,
					'' AS umur, YEAR(curdate()) - YEAR(MP.birthdate) AS umur1 , (CASE MP.gender WHEN 0 THEN 'L' WHEN 1 THEN 'P' END) AS gender,
					TL.id_kmr_from, TL.id_bed_from, MKF.name AS kelas_from,
					TL.indate, TL.id_reg_kmr AS no_kamar, TL.id_bed,
					MKM.`description` AS kamar, MK.name AS kelas, TR.id_icd, MD.name AS dokter,
					TR.penanggung, MC.name AS company, MP.birthdate,
					MIN(DATE(TL.`indate`)) AS tgl_masuk, MAX(DATE(TL.`outdate`)) AS tgl_keluar
					,MP.address,MP.birthplace,MP.pid_num
				FROM (
					SELECT 	MM.id_reg,
