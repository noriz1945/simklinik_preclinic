<?php
class D_Gdf_purchase_order_batal_closing extends ci_model
{
  ////////////////////////////
  function dlistrpo($tglmulai_set,$tglakhir_set){
    $query=$this->db->query("SELECT a.* FROM gdf_purchase_order a 
    WHERE DATE_FORMAT(a.tanggal_closing,'%Y-%m-%d') BETWEEN '$tglmulai_set' AND '$tglakhir_set' AND a.status IN('0','2')
  ...
  function list_detail_closing($id_por){
    ...
    FROM gdf_purchase_order_det a 
    WHERE a.id_purchase_order='$id_por' AND a.status IN('1','2','3')
  ...
  function cdk_kartu_stok(){
    $query=$this->db->query("SELECT id_kode FROM gdf_kartu_stok WHERE kode='BTL' ORDER BY created DESC limit 1");
    return $query->row();
  }
