...
<input type="hidden" class="form-control input-default" id="id_reg" name="id_reg" value="<?php echo $id_reg; ?>">
  ...
  <hr>
  <h5><b>FORMULIR KRITERIA PASIEN KELUAR PICU</b></h5>
  <div class="table-responsive">
     <table class="table table-bordered" style="width:100%">                        
        <tbody id="tbriwakpmpicu"></tbody>
    </table>
  </div>
  <br>
  <div id="row">
  ...
  <div class="col-sm-1">
  <button class="btn btn-success send_news_1_akpm" type="button">Simpan <i class="fa fa-check"></i></button>
  </div>
  </div>
  </div>
  ...
  <div class="table-responsive">
     <table class="table table-bordered" style="width:100%">                          
        <tbody id="tbriwa2" class="tbriwa3kpm"></tbody>
    </table>
  </div>
...
$("#id_frmkepe_kpmpicu").click(function(){
  var idreg_set = $('#id_reg').val();
  listmst(idreg_set);
});
...
function listmst(idreg_set){
  var pj = '<?php echo $creator; ?>';
    $.ajax({
      url : baseUrl+"formkeperawatan/frm_kpm_picu/mst",
      method : "POST",
      ...
      success: function(data){
        ...
        $(".send_news_1_akpm").click(function(e){
          ...
          $.ajax({
            url : baseUrl+"formkeperawatan/frm_kpm_picu/save_news",
            method : "POST",
            ...
            success: function(data){
              ...
              Swal.fire('Berhasil!', 'Formulir Kriteria Pasien Keluar picu', 'success');
            }
          });
          listmst(idreg_set);
          return false;
    ...
    $("#report_a").click(function(){
      var idreg_set     = $('#id_reg').val();
      $.ajax({
            url : baseUrl+"formkeperawatan/frm_kpm_picu/mst_rep",
            method : "POST",
            data : { idreg_set:idreg_set},
            //async : false,
            dataType : 'json',
        ...
        let sumnailainya_1 = 0;
          ...
          line_1_det +="</td></tr>"
              ...
              $('.tbriwa3kpmpicu').html(line_1_det2+line_1_det);
