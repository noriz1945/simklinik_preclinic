		...
		<div class="modal-content">
          ...
          <div class="col-md-6" style="padding:50px;">
            ...
            <div class="form-group row">
              <label class="col-sm-3 col-form-label">ID ICD 10 (Utama)</label>
              <div class="col-sm-9">
                <div class="ui-widget">
                  <input id="id_icd[0]" name="id_icd[0]" class="form-control">
                </div>
              </div>
            </div>
            
            <div class="form-group row">
              <label class="col-sm-3 col-form-label">ID ICD 10 (Kedua)</label>
              <div class="col-sm-9">
                <div class="ui-widget">
                  <input id="id_icd[1]" name="id_icd[1]" class="form-control">
                </div>
              </div>
            </div>
...
/*
  function get_cppt_new(id_cpptx) {
    $.get("<?php #echo base_url('soap/epoli/get_view_cppt/') ?>" + id_cpptx)
      .done(function(data) {
        //alert("Data Loaded: " + data);
$('#ModalBodyCppt').html(data);
...
$(function() {
  $("#id_icd\\[0\\]").autocomplete({
   ...
   // select: function(event, ui) {
		//	$("#id_icd\\[0\\]").val(ui.item.desc);
    //}
  });

	$("#id_icd\\[1\\]").autocomplete({
    source: "<?php echo base_url('rekam_medis/inner_get_data_autocomplet_icd_ten'); ?>",
		appendTo : "#modal-body-upl",
    minLength: 2,
    select: function(event, ui) {
      //$("#jenis_obat").val(ui.item.jenis_obat);
			//$("#id_icd\\[1\\]").val(ui.item.name);
    }
  });
});
