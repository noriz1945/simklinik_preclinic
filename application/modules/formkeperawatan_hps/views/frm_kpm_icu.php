...
function listmst(idreg_set){
  var pj = '<?php echo $creator; ?>';
    $.ajax({
      url : baseUrl+"formkeperawatan/frm_kpm_icu/mst",
      method : "POST",
      ...
      success: function(data){
        ...
        var line_1_det='';
         for (irestind = 0; irestind < data.length; irestind++){ 
          ...
          for (irestind2 = 0; irestind2 < data[irestind].rs_1.length; irestind2++){
            ...
            var diagnosa = data[irestind].rs_1[irestind2].diagnosa_penyakit;
            line_1_det +="<tr><td style='vertical-align : middle;text-align:left;' colspan='2'>"+data[irestind].rs_1[irestind2].indikator+"</td>"
            +"<td colspan='2'><select class='form-control selyt' id='12jam_1a[]' name='12jam_1a[]' attr-data-idmst='"+data[irestind].rs_1[irestind2].id+"'><option value='0' "+nilai_0_set+" default>Pilih</option><option value='1' "+nilai_1_set+">Ya</option><option value='2' "+nilai_2_set+">Tidak</option></select></td>"
            +"</tr>";
          }
          line_1_det +="</td></tr>";
         }
         line_2_det = "<tr>"
         +"<td colspan='1' style='vertical-align : middle;text-align:center;'><b>DIAGNOSA PENYAKIT</b></td>"
         +"<td style='vertical-align : middle;text-align:center;' colspan='4'><textarea class='form-control' id='diagnosapenyakit' name='diagnosapenyakit' placeholder='Diagnosa Penyakit'>"+diagnosa+"</textarea></td>"
         +"</tr>";
        ...
        $(".send_news_1_a").click(function(e){
          e.preventDefault();
          var idreg_set             = $('#id_reg').val();
          var diagnosa_penyakit     = $('#diagnosapenyakit').val();
          //set row 1
          ...
          $.ajax({
            url : baseUrl+"formkeperawatan/frm_kpm_icu/save_news",
            method : "POST",
            data : { idreg_set:idreg_set,jam12_1_set:jam12_1_set,jam12_1_idmst_set:jam12_1_idmst_set,diagnosa_penyakit:diagnosa_penyakit},
            //async : false,
            dataType : 'json',
            success: function(data){
              $('select').prop('selectedIndex', 0);
              Swal.fire('Berhasil!', 'Formulir Kriteria Pasien Masuk ICU', 'success');
            },
              error: function(xhr, ajaxOptions, thrownError) {
                show_my_error_message2(xhr.responseText,xhr.status,xhr.statusText);
              }
    ...
    $("#report_a").click(function(){
      var idreg_set     = $('#id_reg').val();
      $.ajax({
            url : baseUrl+"formkeperawatan/frm_kpm_icu/mst_rep",
            method : "POST",
            data : { idreg_set:idreg_set},
            //async : false,
            dataType : 'json',
        ...
        let sumnailainya_1 = 0;
          ...
          line_1_det +="</td></tr>"
              ...
              $('.tbriwa3').html(line_1_det2+line_1_det);
