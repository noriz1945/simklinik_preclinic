...
class Pembayaran extends MX_Controller
{
	...
	function add_tindakan_act($redir="",$id_reg="")
	{
		...
		$data_tindakan['creator'] = $this->session->userdata['sp']->login_name;
	...
	public function buat_invoice()
	{
		...
		$data_trx_reg_inv['total_cc1'] = $this->input->post('total_cc1');
		...
		if($ada_paket==true)
		{
			...
			///////////////////LOG SYSTEM
			$infoset     = "PEMBAYARAN ".$this->session->userdata['sp']->login_name.' -> '.date('Y-m-d H:i:s');
			$data_log = array(
				'type' 	        =>	"9",
				'attempt_1'     =>	base_url(),
		...
		if(!$debug) redirect('pembayaran?id_inv='. $new_id_inv . '&id_ret_dp=' .$new_id_ret_dp);
	}
	...
	{
		$user_set = $this->session->userdata['sp']->login_name;
	...
	{ 
		...
		$refund_created_by  = $this->session->userdata['sp']->login_name;
	...
	function opening_kasir_act()
	{
		...
		$this->db->trans_begin();
		$data_opening['login_name']   = $this->session->userdata['sp']->login_name;
