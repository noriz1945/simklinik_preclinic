...
class prosesor {
	...
	function get_all_rujukan($noka)
	{
		...
		### PHASE 2 >>> cari rujukan dari Klinik/Puskesmas -------------
		if($multi_rujukan=="")
			$data = $this->wsv->rujukan_search_pcare_multi_by_noka($noka);
	...
	function insert_sep($rujukan,$perjanjian,$other=array(),$peserta,$noSuratKontrol='',$flag_kunjungan=0)
	{
		...
		### --- klsRawat --- ###
		$klsRawatHak		= $peserta->response->peserta->hakKelas->kode;
		$klsRawatNaik		= "";
		$pembiayaan			= "";
		$penanggungJawab	= "";
		...
		*/
        ...
        ### --- flag_kunjungan --- ###
        ## 1 = datang pertama kali dari faskes 1
        ## 2 = datang pertama kali dari faskes 2
        ## 3 = konsul ke dr.spesialis lain
        ## 4 = pasien kontrol ulang ke dokter yang sama
        ## 5 = pasien post rawat
		if($flag_kunjungan==1 || $flag_kunjungan==2) ## 1 = datang pertama kali dari faskes 1
		{
            $tujuanKunj		= '0';
            $flagProcedure  = '';
            $kdPenunjang	= '';
			...
			$noSurat		= '';
			$kodeDPJP		= '';
			$dpjpLayan		= $this->get_id_doctor_smarthis2bpjs($perjanjian['id_dokter']);
		}
        elseif($flag_kunjungan==3) ## 3 = konsul ke dr.spesialis lain
		{
            $tujuanKunj		= '0';
            $flagProcedure  = '';
            $kdPenunjang	= '';
        ...
        elseif($flag_kunjungan==4) ## 4 = pasien kontrol ulang ke dokter yang sama
		{
            $tujuanKunj		= '2';
            $flagProcedure  = '';
            $kdPenunjang	= '';
			...
			$kodeDPJP		= $this->get_id_doctor_smarthis2bpjs($perjanjian['id_dokter']);
			$dpjpLayan		= '';
		...
		/*
		$response = $this->wsv->sep_insert(			$noKartu,$tglSep,$ppkPelayanan,$jnsPelayanan,$klsRawat,
																						$noMR,$asalRujukan,$tglRujukan,$noRujukan,$ppkRujukan,
																						$catatan,$diagAwal,$tujuan,$eksekutif,$cob,
																						$katarak,$lakaLantas,$penjamin,$tgl_kejadian,$ket_kll,
																						$suplesi,$no_sep_suplesi,$kd_propinsi,$kd_kabupaten,$kd_kecamatan,
																						$no_skdp,$kd_dpjp,$noTelp,$user);
		*/
		$response = $this->wsv->sep_2_0_insert(	$noKartu,$tglSep,$ppkPelayanan,$jnsPelayanan,$klsRawatHak,
	...
	function get_monitor_kunjungan_terakhir_peserta_by_rujukan($noka,$noRujukan)
	{
		...
		if(isset($data->response->histori[0]->noRujukan))
		{
			foreach($data->response->histori as $k => $v)
			{
				if($v->jnsPelayanan == 2 && trim($v->noRujukan) == trim($noRujukan))
				{
					$return = $v;
				}
			}
	...
	public function get_perjanjian_today_by_id_pasien($id_pasien)
	{
		$sql = "SELECT 	a.*,d.* 
						...
						WHERE 	a.id_pasien='".$id_pasien."' AND date(a.bookdate)='".$this->today."' AND a.id_reg IS NULL";
		$query = $this->dbhis->query($sql);
		$jum_data = $query->num_rows();
		$rs = $query->result_array();
		$data = array(
	...
	function rencana_kontrol_insert_if_not_exist($nosep,$kodeDokter,$poliKontrol,$tgl_rencana_kontrol)
	{
		#$pasien 	= $this->get_mst_pasien_by_id_pasien($id_pasien);
		#$noka 		= $pasien['asm_id'];
		
		#$last_reg = $this->get_last_reg_rwj_bpjs($id_pasien,$tipe_rawat);
		#if(empty($last_reg)) return;
		
		#$id_dokter 	= $last_reg['id_dokter'];
		#$id_poli 		= $last_reg['id_unit'];
		
		#$nosep 		= $this->get_nosep_by_date($noka,$sep_date);
		#if($nosep=='') return;
		
		#$kodeDokter 	= $this->fs->get_id_doctor_smarthis2bpjs($id_dokter);
		#$poliKontrol 	= $this->fs->get_id_poly_smarthis2bpjs($id_poli);
		
		$noSuratKontrol = $this->cek_is_surat_rencana_kontrol_exist($filter="2",$tgl_rencana_kontrol);
		if($noSuratKontrol != '')
		{
			#return $noSuratKontrol;
			$hapus = $this->surat_rencana_kontrol_hapus($noSuratKontrol);
		}
		...
		$response = $this->wsv->rencana_kontrol_insert($noSEP,$kodeDokter,$poliKontrol,$tglRencanaKontrol,$user);
		
		$noSuratKontrol = $response;
		### $noSuratKontrol = $response->response->noSuratKontrol;
		return $noSuratKontrol;		
	}
	
	function cek_is_surat_rencana_kontrol_exist($filter="2",$tglRencanaKontrol='')
	{
		if($tglRencanaKontrol == '') $tglRencanaKontrol = $this->today;
		
		#$ninety_days_ago = date('Y-m-d',mktime(0,0,0,date('m'),(date('d')-90),date('Y') ));
		$data = $this->wsv->rencana_kontrol_data_list($this->today,$this->today,$filter);
		...
		if(isset($data))
		{
			...
			{
				if(is_array($data->response->list))
				{
					foreach($data->response->list as $k => $v)
					{
						if($v->tglRencanaKontrol == $tglRencanaKontrol)
						{
							$noSuratKontrol = $v->noSuratKontrol;
							break;
						}
					}
				}
	...
	function surat_rencana_kontrol_hapus($noSuratKontrol)
	{
		$user	= $this->config->item('apem_user_app');
		$data = $this->wsv->rencana_kontrol_delete($noSuratKontrol,$user);
		return $data->metaData->code; 
	}
	
	function get_last_reg_rwj_bpjs($id_pasien,$tipe_rawat='RJ')
	{
		if($id_pasien=='') return '';
		...
		if($tipe_rawat=='RI')
		{
			$sql 		= "	SELECT 	a.*,DATE(a.regdate) AS datereg,b.`id_dokter`,b.`id_unit`
									FROM 		trx_reg a, trx_reg_unit b
									WHERE 	a.id_reg=b.id_reg AND a.rwjn=1 AND a.id_pasien=".$id_pasien." 
													AND a.`id_asuransi`='".$id_asuransi."'
									ORDER BY a.regdate DESC 
	...
	function get_nosep_by_date($noka,$sep_date)
	{
		$return = ''; ## >> init
        $today = $this->today;
		$ninety_days_ago = date('Y-m-d',mktime(0,0,0,date('m'),(date('d')-90),date('Y') ));
		$data = $this->wsv->monitoring_data_histori_pelayanan_peserta($noka,$ninety_days_ago,$today);
		if(isset($data->response->histori[0]->noSep))
		{
			foreach($data->response->histori as $k => $v)
			{
				if($v->jnsPelayanan == 2 && $v->tglSep == $sep_date)
				{
					$return = $v->noSep;
				}
			}
	...
	function rollback_booking($id_trx)
	{
		$this->dbhis->delete('trx_reg_book', array('id_trx' => $id_trx));
		$this->dbsupp->delete('trx_reg_book_ext', array('id_trx' => $id_trx));
	}
	
	function cek_is_sudah_finger($noka)
	{
		$return = false;
		$data = $this->wsv->sep_finger_print($noka,$this->today);
		if(isset($data->response->kode))
		{
